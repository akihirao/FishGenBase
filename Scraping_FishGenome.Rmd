...
AGSDB_fish <- read_csv("AGSDB_fish.csv")
no_AGSDB_fish <- nrow(AGSDB_fish)
AGSDB_C_vec <- vector()
AGSDB_genus_vec <- vector()

for(i in 1:no_AGSDB_fish){
  AGSDB_C_values <- strsplit(AGSDB_fish$'C-value'[i], "-") %>% unlist
  AGSDB_C_vec[i] <- as.numeric(AGSDB_C_values[1])
  AGSDB_species_unlist <-strsplit(AGSDB_fish$Species[i]," ") %>% unlist
  AGSDB_genus_vec[i] <- AGSDB_species_unlist[1]
}
AGSDB_fish <- AGSDB_fish %>% mutate(Genus=AGSDB_genus_vec, C_value=AGSDB_C_vec)

AGSDB_genus_ave_genome_size <- tapply(AGSDB_fish$C_value,AGSDB_fish$Genus, mean)
AGSDB_no_genus_avail <- tapply(AGSDB_fish$C_value,AGSDB_fish$Genus, length)
```
...
for(i in 1:no_fish){
  ...
  genus_species_name <- strsplit(fishes[i,]$Scientific_name, " ") %>% unlist 
  ...
  if(!(identical(AGSDB_genus_ID,integer(0)))){
    AGSDB_genus_genome_size_vec[i] <- round(as.numeric(AGSDB_genus_ave_genome_size[AGSDB_genus_ID]),digits=2)
