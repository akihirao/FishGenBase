...
# Family2OrderList in fish species
Family2Order <- read_csv("Family2OrderList.csv")
...
for(i in 1:no_fish){
	...
	if(Category == "Fish"){
	  ...
	  target_Family_ID <- which(Family2Order$Family==Family_name)
	  
	  Phylum_name <- Family2Order$Phylum[target_Family_ID]
	  if(identical(Phylum_name,character(0))){
	    Phylum_name <-NA
	  }
	  
	  Class_name <- Family2Order$Class[target_Family_ID]
	  if(identical(Class_name,character(0))){
	    Class_name <-NA
	  }
	  
	  Order_name <- Family2Order$Order[target_Family_ID]
...
FRA200List_Latin <- FRA200List %>% 
  mutate(Phylum = phylum_name_vec, Class = class_name_vec, Order = order_name_vec, Family = family_name_vec, Scientific_name = scientific_name_vec) %>%
  arrange(Category, Phylum, Class, Order, Family, Scientific_name) %>% 
